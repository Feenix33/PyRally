...
def printHelp():
    ...
    print ("    -i    Milestone ID as written") 
    print ("    -q    Quiet")
    print ("    -s    Do no print stories")
    print ("    -t    Do no print team features and stories")
    print ("    -h    Help")
...
def main(args):
    ...
    for opt in options:
        if opt[1] == "s":
            ...
            bQueryName = False #query is by ID
            bFullLabel = True
        elif opt[1] == "h":
            printHelp()
            sys.exit(0)
    ...
    for arg in args:
        ...
        if bQueryName:
            queryString = 'Milestones.Name contains "%s"' % (sMile)
        else:
            queryString = 'Milestones.FormattedID = "%s"' % (sMile)
        ...
        if response.resultCount == 0:
            ...
            with open (fileName, 'w', newline='', encoding='utf-8') as csvfile:
                ...
                for item in response:
                    ...
                    if bPrintTeamFeatures and hasattr(item, "Children"):
